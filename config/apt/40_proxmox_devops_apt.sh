...
for line in "${QMS[@]}"; do
  ...
  for net in "${QM_NETWORKS[@]}"; do
    ...
    [ -n "$QM_NET_BRIDGE" ] && [ "$QM_NET_BRIDGE" != "0" ] && QM_NET_BRIDGE_PRINT=",bridge=$QM_NET_BRIDGE" || QM_NET_BRIDGE_PRINT=""
    [ -n "$QM_NET_MAC" ] && [ "$QM_NET_MAC" != "0" ] && QM_NET_MAC_PRINT="=$QM_NET_MAC" || QM_NET_MAC_PRINT=""
    [ -n "$QM_NET_VLAN" ] && [ "$QM_NET_VLAN" != "0" ] && QM_NET_VLAN_PRINT=",tag=$QM_NET_VLAN" || QM_NET_VLAN_PRINT=""
    qm set "$QM_ID" "--$QM_NET_NAME" "virtio${QM_NET_MAC_PRINT},firewall=0,mtu=1500${QM_NET_BRIDGE_PRINT}${QM_NET_VLAN_PRINT}"
...
for line in "${PCTS[@]}"; do
  ...
  for net in "${PCT_NETWORKS[@]}"; do
    ...
    [ -n "$PCT_NET_BRIDGE" ] && [ "$PCT_NET_BRIDGE" != "0" ] && PCT_NET_BRIDGE_PRINT=",bridge=$PCT_NET_BRIDGE" || PCT_NET_BRIDGE_PRINT=""
    [ -n "$PCT_NET_IP" ] && [ "$PCT_NET_IP" != "0" ] && PCT_NET_IP_PRINT=",ip=$PCT_NET_IP" || PCT_NET_IP_PRINT=""
    [ -n "$PCT_NET_IP6" ] && [ "$PCT_NET_IP6" != "0" ] && PCT_NET_IP6_PRINT=",ip6=$PCT_NET_IP6" || PCT_NET_IP6_PRINT=""
    [ -n "$PCT_NET_VLAN" ] && [ "$PCT_NET_VLAN" != "0" ] && PCT_NET_VLAN_PRINT=",tag=$PCT_NET_VLAN" || PCT_NET_VLAN_PRINT=""
    pct set "$PCT_ID" "--$PCT_NET_NAME" "name=$PCT_NET_ALIAS,firewall=0,mtu=1500${PCT_NET_BRIDGE_PRINT}${PCT_NET_IP_PRINT}${PCT_NET_IP6_PRINT}${PCT_NET_VLAN_PRINT}"
