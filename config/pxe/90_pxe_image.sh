...
if [ -e /bin/apt ]; then
  ...
  if grep -q Debian /proc/version; then
    ...
    mksquashfs / /srv/pxe/debian/x86_64/pxeboot.img -comp zstd -Xcompression-level 4 -b 1M -progress -wildcards \
      -e "boot/*" "cidata*" "dev/*" "etc/fstab*" "etc/crypttab*" "proc/*" "sys/*" "run/*" "mnt/*" "share/*" "srv/pxe/*" "media/*" "tmp/*" "swap/*" "var/tmp/*" "var/log/*" "var/cache/apt/*" "var/lib/cloud/*"
  ...
  mksquashfs / /srv/pxe/arch/x86_64/pxeboot.img -comp zstd -Xcompression-level 4 -b 1M -progress -wildcards \
    -e "boot/*" "cidata*" "dev/*" "etc/fstab*" "etc/crypttab*" "proc/*" "sys/*" "run/*" "mnt/*" "share/*" "srv/pxe/*" "media/*" "tmp/*" "swap/*" "var/tmp/*" "var/log/*" "var/cache/pacman/pkg/*" "var/lib/cloud/*"
fi
