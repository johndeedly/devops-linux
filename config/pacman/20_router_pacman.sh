...
LC_ALL=C yes | LC_ALL=C pacman -S --noconfirm --needed net-tools dnsmasq iptraf-ng ntp step-ca step-cli nginx nfs-utils \
...
$(</var/lib/cloud/instance/provision/pacman/20_router_pacman/grub.cfg.default)
EOF
  ...
  "ports": [
    {
      ...
      "referrals": [],
      "subsystems": [
        "iqn.2018-12.internal.pxe:target"
      ]
    }
  ],
  "subsystems": [
    {
      ...
      "nqn": "iqn.2018-12.internal.pxe:target"
    }
  ]
}
EOF
...
Port=3260
EOF
tee /etc/systemd/dnssd/nvmet.dnssd <<EOF
[Service]
Name=%H
Type=_nvmet._tcp
Port=8009
