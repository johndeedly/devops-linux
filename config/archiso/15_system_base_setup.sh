...
if [ -d "/iso/archiso/pkg" ] && [ -n "$(find /iso/archiso/pkg -type f)" ]; then
    ...
    pacman -Swp --logfile "/dev/null" --cachedir "/dev/null" wget libguestfs qemu-base jq yq libisoburn kexec-tools ansible | while read -r line; do
        echo "$line"
        echo "$line".sig
...
LC_ALL=C yes | LC_ALL=C pacman -S --noconfirm wget libguestfs qemu-base jq yq libisoburn kexec-tools ansible
